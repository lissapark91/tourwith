...
public class EchoHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        //sessions.getAttribute("LOGIN_USER");
        //logger.info(session.getId() + "님이 접속했습니다.");
        logger.info(member.getNick() + "님이 접속했습니다.");
            ...
            if (messageVO.getType().equals("all")) {
                if (!session.getId().equals(webSocketSession.getId())) {
                    webSocketSession.sendMessage(
                            new TextMessage(member.getNick() + " ▶ " + messageVO.getMessage()));
                }
            } else {
                ...
                if (messageVO.getTo().equals(hostName)) {
                    webSocketSession.sendMessage(
                            new TextMessage(
                                    ...
                                    + member.getNick() + "▶ " + messageVO.getMessage()
//                                    + session.getRemoteAddress().getHostName() + "▶ " + messageVO.getMessage()
         ...
         * Payload : 사용자가 보낸 메시지
         */
        logger.info(member.getNick() + "님의 메시지 : " + message.getPayload());
    }
